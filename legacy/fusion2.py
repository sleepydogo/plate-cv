...
kernel = np.ones((10, 10), np.uint8)  # Puedes ajustar el tamaño y la forma del kernel según tus necesidades



imagen_dilatada = cv2.dilate(bordes, kernel, iterations=1)

# Aplica la erosión
imagen_erodada = cv2.erode(imagen_dilatada, kernel, iterations=1)


cv2.imshow('Imagen Original', bordes)
cv2.imshow('Imagen Dilatada', imagen_erodada)
cv2.waitKey(0)
